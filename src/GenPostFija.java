    ...
    public List<Token> convertir() {
        ...
        for (int i = 0; i < infija.size(); i++) {
            ...
            if (t.Keyword()) {
                ...
                while (!stack.isEmpty() && stack.peek().tipo != TipoToken.LEFT_PAREN) {
                    Token temp = stack.pop();
                    postfija.add(temp);
                }
                if (!stack.isEmpty()) {
                    if (stack.peek().tipo == TipoToken.LEFT_PAREN) {
                        stack.pop();
                    }
                }
                if (controlStructure && infija.get(i + 1).tipo == TipoToken.LEFT_BRACE) {
                ...
                while (!stack.isEmpty() && stack.peek().tipo != TipoToken.LEFT_BRACE) {
                    Token temp = stack.pop();
                    postfija.add(temp);
                }
                postfija.add(t);
            ...
            else if (t.tipo == TipoToken.RIGHT_BRACE && controlStructure) {
